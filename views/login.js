...
export default class Login extends React.Component {
  ...
  saveServer() {
    if (
      this.state.newServer.clientID !== '' &&
      this.state.newServer.clientSecret !== '' &&
      ...
      this.setState(
        ...
        () => {
          Linking.getInitialURL().then((url) => {
            this.displayError(
              `Please make sure to add this "${url}" to your authorized redirects`,
            );
          });
        },
      );
      ...
      this.displayError(
        'Please fill out the domain, client ID, and client secret',
      );
    }
  }
      ...
      <ScrollView style={Style.view}>
        <View style={Style.card}>
          ...
          <Text style={{color: COLORS.text}}>
            Until somebody implements proper{' '}
            <LinkText url="https://oauth.net/2/grant-types/implicit/">
              implicit OAuth
            </LinkText>
            , you're going to have to apply for app keys with your
            ruqqus-compatible server admins.
          </Text>
          ...
          {this.state.newServer.domain ? (
            <Text style={{color: COLORS.text}}>
              Please tap{' '}
              <LinkText
                url={`https://${this.state.newServer.domain}/settings/apps`}>
                here
              </LinkText>{' '}
              to apply for the client ID and client secret
            </Text>
          ) : null}
            ...
            return (
              <View
                ...
                <Text
                  ...
                  <Text style={{color: COLORS.muted}}>Client ID:</Text>{' '}
                  {server.clientID}
                </Text>
